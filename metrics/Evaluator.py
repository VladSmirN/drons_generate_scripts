    ...
    def GetPascalVOCMetrics(self,
        ...
        for c in classes:
            ...
            for d in detections:
                if d[1] == c:
                    ds[d[0]] = ds.get(d[0], []) + [d]
            groundTruthsForClass = []
            [groundTruthsForClass.append(gt) for gt in groundTruths if gt[1] == c]
            FN = np.zeros(len(groundTruthsForClass))

            for i in range(len(groundTruthsForClass)):
                dect = ds[groundTruthsForClass[i][0]] if groundTruthsForClass[i][0] in ds else []
                status = 1
                for j in range(len(dect)):
                    iou = Evaluator.iou(dect[j][3], groundTruthsForClass[i][3])
                    if iou >= IOUThreshold:
                        status = 0
                        break
                FN[i] = status
